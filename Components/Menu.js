...
const Menu = () => {
  const menuRef = useRef(null);
  const [isVisible, setIsVisible] = useState(false);
  const [currentIndexes, setCurrentIndexes] = useState(Array(6).fill(0));
  const [opacities, setOpacities] = useState(Array(6).fill(1));

  // ✅ Memoized menuItems to prevent recreation on every render
  ...
  // ✅ Image slideshow logic (memoized)
  const handleImageTransition = useCallback((index) => {
    setOpacities((prev) => {
  ...
  // ✅ Setup slideshow intervals
  useEffect(() => {
    const intervals = menuItems.map((item, index) =>
      setInterval(() => handleImageTransition(index), item.delay)
    );
    return () => intervals.forEach(clearInterval);
  }, [handleImageTransition, menuItems]);

  // ✅ Animate menu when visible
  useEffect(() => {
  ...
  return (
    <div id="menu" className="bg-white text-black py-8" ref={menuRef}>
      <div className={`max-w-7xl mx-auto px-4 transform transition-transform duration-700 ${isVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}>
        <h2 className="text-3xl md:text-4xl font-extrabold text-center mb-12">
